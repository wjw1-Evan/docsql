...
**离线自动补齐(重启反熵修复)**:节点离线/分区期间,其它节点的写不会实时补发;但离线节点**重启时会自动修复**——对比各节点的表数据摘要,发现分歧即采纳集群多数方(在线幸存节点)的最新快照(极端情况无多数方时,按「数据较多者优先」的确定性规则选出参考方),离线期间错过的增删改全部自动补齐,全网恢复一致。两点边界:没有行级合并,分歧中**少数方/数据较少一方独有**的写会被参考方快照覆盖(多数方在线节点上的数据为准);修复由重启触发,仅网络分区而各节点未重启时不自动收敛(任一分歧节点重启即收敛)。
