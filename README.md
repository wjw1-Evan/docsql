...
- **浅色 / 深色主题**:工具栏按钮一键切换,首次进入跟随系统偏好,手动选择后记忆;
- **多语言(中文 / English)**:工具栏地球按钮一键切换,即时生效且不刷新页面(已打开的标签页
  重渲染、查询编辑器内容保留),首次进入跟随浏览器语言,手动选择后记忆;
...
| Web | **DocSQL Studio**(SSMS 风格管理控制台,纯管理工具、自身不存数据,默认连接并管理指定节点):对象资源管理器(表/列/索引/键 + 系统视图)、多标签查询编辑器(SQL 高亮/F5 执行/Ctrl+F5 分析/批量多结果集)、数据网格(排序/分页/行内编辑与删除——按主键定位生成 UPDATE/DELETE,自增列只读、可勾选置 NULL;主键列缺失的表不提供行编辑)、新建表 / 插入文档(参考 mongo-express:列编辑网格建表——类型含 GUID 时序主键、JSON 文档插入,支持批量与表外字段)、服务器仪表盘、集群状态页、日志页(数据/同步/错误,含各节点来源)、备份管理页(备份状态/文件列表/立即备份/一键恢复——恢复需输入完整文件名确认)、节点切换(默认管理节点 ↔ 任意 `DOCSQL_PEERS` 节点)、浅色/深色主题(工具栏切换,首次跟随系统)、多语言中文/English(工具栏切换,即时生效);REST API(/api/sql /api/parse /api/meta /api/stats /api/cluster /api/logs /api/backup,其中数据端点 /api/sql /api/meta /api/stats /api/backup 可带 `node` 参数指定目标节点) |
...
四个 NuGet 包发布在 GitHub Packages(`net10.0`,版本随 `v*` tag 发布,当前 `0.5.0`):
