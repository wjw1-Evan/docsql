...
| SQL | CREATE/ALTER/DROP TABLE+INDEX、INSERT(多行/RETURNING)、UPDATE/DELETE(RETURNING)、SELECT(WHERE/ORDER/LIMIT/OFFSET/GROUP BY+HAVING/COUNT/SUM/AVG/MIN/MAX/JOIN:INNER/LEFT/CROSS/USING/子查询派生表/UNION(ALL)/IN)、事务 BEGIN/COMMIT/ROLLBACK、PRIMARY KEY/UNIQUE/NOT NULL/AUTOINCREMENT、GUID 主键(UUIDv7 时序有序自动生成)、JSON 函数(JSON_EXTRACT/JSON_TYPE/JSON_VALID:文档点读路径 `$.a.b[0]`)、**多列(复合)索引**(`CREATE INDEX … ON t (a, b)`,复合 UNIQUE 判重;前导列等值探测)、information_schema、sqlite_master 兼容视图、PRAGMA 兼容 |
