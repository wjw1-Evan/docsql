...
./deploy/run-tests.sh           # 本地构建镜像 + 部署测试(多节点 19 项 + 单节点 14 项)
```
...
- **对象资源管理器**(左栏树):服务器 → 表(列含 PK/UQ/NN/AI 徽章、索引、键)→ 每表可双击打开数据网格;系统视图(`information_schema.*`、`sqlite_master`)
- **查询工作台**(多标签文档):SQL 语法高亮 + 行号编辑器,F5 执行 / Ctrl+F5 仅语法分析 / 执行所选;多语句批次依次执行并逐结果集呈现(网格 + "(N 行受影响)" 消息页 + 总耗时);表头点击排序
- **右键任务**:新建查询、选择前 1000 行、查看数据、编写 CREATE/DROP 脚本、删除表
...
| 网络 | 自定义二进制协议 v1(预留拓扑版本/重定向字段)、REQ_AUTH token 认证、REQ_PROMOTE 故障转移提升 |
| Web | **docsql Studio**(SSMS 风格管理控制台):对象资源管理器(表/列/索引/键 + 系统视图)、多标签查询编辑器(SQL 高亮/F5 执行/Ctrl+F5 分析/批量多结果集)、数据网格(排序/分页/删行)、服务器仪表盘;REST API(/api/sql /api/parse /api/meta /api/stats) |
...
| 集群 | 对等集群(`DOCSQL_PEERS`:任意节点可写,SQL 写入扇出至全部对等节点)、主从复制(写转发)、只读副本、PROMOTE 故障转移 |
...
- Rust:单元 + SQL 集成 + 协议 + 端到端 + 复制故障转移 + 批处理/目录元数据(亦在本地 Docker 构建内作为门禁执行)
- Docker:compose 双 profile 部署测试全绿——多节点 19 项(3 节点对等集群:任意节点写入/多向 SQL 复制/事务回滚/一致性收敛/Web 控制台)+ 单节点 14 项(SQL 读写/事务回滚/容器重启持久性/与集群隔离/Web 控制台)
- .NET:xUnit(ADO.NET Client 34 项 + EF Core 22 项:CRUD/LINQ/Include/Savepoint/集群/加密传输)
- CI(GitHub Actions,push/PR 触发):`cargo fmt` + `cargo clippy -D warnings` + `cargo test` + `dotnet test` 全过 → 构建镜像 → main 分支另跑同一套部署测试(19 + 14 项)
...
| `cluster` | node-a + node-b + node-c 对等集群(任意节点可读写,SQL 写入自动扇出至 `DOCSQL_PEERS`)+ web 控制台 | 17601-17603 / 17700 |
...
部署测试(`./deploy/run-tests.sh`,同时拉起两个 profile):多节点 19 项(任意节点写入/多向 SQL 复制/事务回滚/一致性收敛/Web 控制台)+ 单节点 14 项(SQL 读写、事务回滚、容器重启后数据持久、与集群的数据隔离、Web 控制台)。
