...
./deploy/run-tests.sh            # 同时拉起 single + cluster 两个 profile:多节点 76 项 + 单节点 34 项
```
...
deploy/                 # docker-compose.yml(本地开发,源码构建)、docker-compose.prod.yml(生产,GHCR 镜像 + .env);均含 single/cluster/join 三个 profile(join = 第四数据节点,node-d,验证新节点自动同步);测试脚本 single-test.sh(34 项,第 8 章自动备份与恢复演练)与 multinode-test.sh(76 项,含节点离线再上线自动补齐、网络分区与重启收敛、第 12 章新节点加入自动同步)
...
| `engine.rs` | SQL 执行器:AST → 计划 → 执行;约束、事务、SAVEPOINT、RETURNING、DISTINCT、外键检查、批量执行;「写单元」(`Database::write_unit`,借用守卫):多条语句共享一次 WAL fsync(显式事务的 deferred 提交机制复用),server 用它把写与复制记账融合 |
...
| `meta.rs` | 对象浏览器元数据组装(server/storage/totals/tables + system_tables):server 的 REQ_META 帧用该实现组装(控制台对象树的后端数据源由此与 UI 渲染形状稳定);系统表不进 tables/totals,单列在 `system_tables`(name+row_count)供控制台「系统表」只读分支 |
...
| 文件 | 职责 |
|---|---|
| `lib.rs` / `main.rs` | tokio TCP 服务器:SQL 会话、REQ_AUTH 鉴权(客户端 token / 集群 token 双身份 `ConnRole`,复制帧仅 peer)、REQ_PROMOTE 故障转移提升、REQ_STATUS 节点状态报告(JSON,含 `replay_failures`=join 队列重放失败计数)、REQ_LOGS 日志报告、REQ_DIGEST/REQ_SQL_SEQ/REQ_CATCHUP(catch-up 复制:摘要探测、带序号复制写、增量拉取)、REQ_BACKUP(备份管理:list/手动触发/restore 整库恢复,需认证;trigger 与 restore 拒只读连接,异步执行)、pub/sub 帧路由、副本协议 |
...
| `backup.rs` | 自动备份与恢复:定时逻辑快照(复用 `dump_script`,dump 在 `lock_engine_for_write` 持 `write_order` 下取、文件 IO 在全部锁释放后做;**首拍仅当无备份或最新备份已超一个间隔**——重启风暴不再把 keep-N 挤成近同快照;文件名时间戳相对目录内现有文件单调递增,时钟回退不破坏名字序=写入序);文件落 `<db>/backups/backup-<UTC毫秒戳>.sql`(.tmp+rename 原子,保留 KEEP 份只删本命名模式);`BackupShared`(running/restore 防重入 + last/restore 状态)经 REQ_STATUS 的 `backup` 字段与 REQ_BACKUP list/trigger/restore 暴露,成败写 sync_log("backup"/"restore" 事件);**恢复 = 逐条语句走正常写路径重放**(`execute_sql` autocommit,每条本地提交+记期刊+扇出;**整场重放持有 write_order**,客户端写不会插进恢复流——插行曾让恢复中途 PK 冲突而失败),完成后做**收敛自愈+验证**:经 catchup_plan/run_catchup 补拉重放期间错过的扇入,再逐一比对可达 peer 摘要,restore 状态暴露 `converged`(true/false;仍分歧的节点靠其重启修复)与 `note`;**跨节点互斥**:发起节点逐个探测 peers 的 REQ_STATUS,他节点恢复进行中即拒;空脚本(空库备份)= 干净的 0 语句 no-op;同名表被覆盖,备份后新建的表保留;文件名严格校验(仅 `backup-*.sql` 裸名,拒分隔符/..),restore 拒只读连接与只读副本;**锁纪律:backup 状态锁从不与 write_order/engine 嵌套**(status_payload 先读 backup 再锁 engine);启动期 sync 门未关(join/repair 中)的 tick/trigger/restore 一律跳过/拒绝 |

**docsql-web**:

| 文件 | 职责 |
|---|---|
| `lib.rs` / `main.rs` | REST API(`/api/sql` `/api/parse` `/api/meta` `/api/stats` `/api/cluster` `/api/logs` `/api/backup` `/api/auth/*`)、`DOCSQL_TOKEN` 认证。**纯管理工具、自身零存储**(不打开任何数据文件):启动参数/`DOCSQL_UPSTREAM`/首个 peer 指定**默认管理节点**,数据端点不带 `node` 时即连接它执行;`DOCSQL_PEERS` 节点探测(PING + REQ_STATUS,集群状态页数据源)兼作节点切换的允许列表,数据端点可带 `node` 参数切换管理目标(白名单仅放行 `DOCSQL_PEERS`,SSRF 防护;连接经 wire 协议 REQ_AUTH——用服务端自身 token,非浏览器提交值;批量 SQL 由 `stmt::split_statements` 逐句发 REQ_SQL,单连接保序;meta/stats 走 REQ_META/REQ_STATUS),未配置管理节点时数据端点 in-band 报配置缺失;**`/api/parse` 恒为本地静态检查**(纯解析,无存储);**`/api/logs`** 返回控制台自身提交语句的审计环(`record_console_sql`,peer=实际执行该语句的节点)+ 并发拉取各节点 REQ_LOGS(读缓冲上限 LOGS_RECV_CAP 4MB,limit 1..=1000 夹紧),不感知 `node`(日志页按来源聚合是它的本意);**`/api/backup`** GET=备份页数据(REQ_BACKUP list 代理,目录/文件列表/上次成败/恢复状态),POST=手动触发(REQ_BACKUP trigger 代理,节点异步执行,结果经 GET 轮询回显;`node` 可来自 query 或 JSON body);**`/api/backup/restore`** POST {file, confirm}=恢复(confirm 必须逐字重复 file,缺失/不一致 400;REQ_BACKUP restore 代理,节点逐条重放快照并扇出全网,异步,进度经 GET 的 restore 字段轮询(含 converged/note 收敛结果);仅接受节点备份目录内的 `backup-*.sql` 裸文件名),节点拒绝 in-band 透传(HTTP 200 + error JSON),传输层失败(不可达/超时)回 502 供脚本区分;**`/api/auth/*`** 控制台账号门(见 `auth.rs`),setup 一次性 / login / logout / status,数据端点凭 HttpOnly 会话 Cookie 或 `DOCSQL_TOKEN` 旁路通过;**Setup 态(未完成首次 setup)且未配 token 时数据端点一律 401**——旧「匿名可达兼容窗口」已收紧:整库替换级操作不得匿名触达,setup/auth 端点本身无需凭据 |
...
| `console.html` | DocSQL Studio 单页 UI(通过 `include_str!` 内嵌进二进制,改完必须重新 `cargo build`);含集群状态页(5s 轮询 `/api/cluster`)、日志页(`openLogsTab`:数据/同步/仅错误类别 + 来源 + 关键词过滤,5s 激活时轮询,同步事件中文标签 SYNC_EVENTS)、备份管理页(`openBackupTab`:备份状态卡/文件列表/「立即备份」/每行「恢复」(输入完整文件名确认后 POST /api/backup/restore,恢复进行中显示进度条),5s 激活时轮询 `/api/backup`)与工具栏节点切换器(`nodesel`:首项为「默认节点」即后端配置的管理目标,api 层统一携带 `node`,选择持久化 localStorage,节点从 peers 消失自动回落默认节点,节点不可达时资源管理器显示 in-band error) |
...
- **全局事务有连接所有权**(`ServerState::tx_owner`):BEGIN 的连接成为 owner,其写语句缓冲在 `tx_pending`,提交时按执行顺序转发给上游/对等节点(整批 journal 追加先合入**一个写单元**落盘,再逐条扇出),回滚即丢弃;**非 owner 连接的写与复制写不并入打开的事务**——像 BEGIN 一样有界排队等待事务关闭(超时报错),否则 owner 的 ROLLBACK 会丢掉别人已确认的写造成静默分叉;COMMIT/ROLLBACK/SAVEPOINT 来自非 owner 直接报错;owner 连接断开时服务端自动 ROLLBACK 其未提交事务并清 `tx_pending`(控制台的短连接因此不会在节点上悬挂事务;cluster join 的 drain 路径持 write_order 直用,复用 `execute_sql` 的 `order_held` 参数防自锁)
- 写执行与扇出经 `write_order` 互斥串行化,保证对等节点按本节点执行顺序应用

### 网络与复制

- 自定义二进制协议 v1(`core/proto.rs`),帧头预留拓扑版本/重定向字段;请求帧:REQ_SQL、REQ_AUTH(token 认证)、REQ_PREPARE/REQ_EXECUTE/REQ_CLOSE_STMT(参数化语句)、REQ_PING、REQ_PROMOTE(故障转移提升)、REQ_STATUS(节点状态 JSON 报告,需认证;web 控制台集群页的探测数据源)、REQ_LOGS(日志报告,需认证;载荷可选 `{"limit":n}` 默认 200 上限 1000,返回查询日志+同步日志两环最新条目,web 日志页数据源)、REQ_BACKUP(备份管理,需认证;`{"action":"list"}` 回 RESP_BACKUP JSON=目录/文件列表/上次成败/恢复状态,`{"action":"trigger"}` 异步触发一次备份,`{"action":"restore","file":…}` 异步整库恢复——逐条走写路径重放并扇出全网,仅接受 `backup-*.sql` 裸文件名;trigger 与 restore 拒只读连接)、REQ_META(对象浏览器元数据,需认证;控制台对象树的数据源,`core/meta.rs` 组装)、REQ_DIGEST(表复制摘要,需认证 + FLAG_REPLICATION;重启反熵修复的探测帧)、REQ_SQL_SEQ(带原点日志序号的复制写,载荷 [seq][node_id][sql];对端应用后记位点且位点推进单调不回退,旧版本对端回错则回退纯 REQ_SQL 重发;join 引导期与 REQ_SQL 同受 sync 门约束——入队即确认,见下)、REQ_CATCHUP(按位点增量拉取原点日志,RESP_CATCHUP 分块 + RESP_AFFECTED(head) 终止;head 在服务前采样、流内容不越过它,保证位点只覆盖本次实发的条目——位点只滞后不超前;服务端另对 (after, head] 做**连续性审计**:批内断号、或流在 head 之前终止(中途被 trim/条目丢失)一律回 RESP_ERROR 而非 RESP_AFFECTED——否则请求方采 head 为位点,静默跳过从未实发的段,该段增量化从此失明;请求方收到错误走快照兜底;重启反熵修复用,见下)、REQ_SUBSCRIBE/REQ_PSUBSCRIBE/REQ_UNSUBSCRIBE/REQ_PUNSUBSCRIBE、REQ_PUBLISH、REQ_PUBSUB(内省/trim)、REQ_SYNC/REQ_HOLD/REQ_RELEASE(新节点加入与重启反熵修复的快照通道,见下),推送帧 RESP_PUSH(仅发给订阅过的连接)。**REQ_SQL 执行全文不截断**:查询日志自行截断显示;曾在执行路径截 512 字符,超过 512 字符的语句被无声截断(控制台发长文档 INSERT 即触发),已修并有回归测试
- 复制两种形态:**对称集群**(`DOCSQL_PEERS` 互相扇出,任意节点可写)与**主从写转发**(`replicate_to` 指向主,副本只读);`PROMOTE` 提升副本为主
- **新节点自动同步(cluster join)**:全新节点(无用户表)配置了 `DOCSQL_PEERS` 即在启动后自动 bootstrap——先经 REQ_STATUS 探测(带 FLAG_REPLICATION),从首个有数据的 peer 发 REQ_SYNC(可带 `DOCSQL_ADVERTISE` 通告自身地址);服务端持自身 `write_order` → 向每个 peer 发 REQ_HOLD(对端 5s 内排空在途写后冻结并注册 joiner,抢不到锁回 busy,60s 看门狗防泄漏)→ **任一 hold 失败整体中止**(部分冻结的网格会漏写,joiner 下轮重试)→ 全网静止时 `Database::dump_script()` 取快照(DDL 全部在前、INSERT 在后,FK 延迟到插入期检查所以建表顺序无关;跳过 `_pubsub_messages`)→ 注册 joiner → REQ_RELEASE 解冻 → 以 ≤4MB 的 RESP_SYNC 块流式回传。joiner 在单事务内整体重放(失败即回滚保持全新)。**加入期间的复制写入队即确认**(`sync_queue`):扇出在源节点写路径内执行,若让扇出在门上等待会占死源节点 write_order、与 REQ_SYNC 互锁(实测过的活锁);快照 → 队列按到达序 → 闭队后直用,构成收敛所需全序;静态 peers 下快照冻结前已扇出的入队写会与快照内容重复,带序号的入队写(REQ_SQL_SEQ,队列携带 origin+seq)在 drain 时跳过位点已覆盖的项(见 repair 段的播种顺序)并随应用推进位点,普通入队写照旧重放、重复仅记日志不致命;bootstrap 的每个终态(Applied/LocalData/放弃/空集群)都必须 drain 队列。**终态 drain 不持锁时自取 write_order 并整场持有**——曾裸信 `order_held` 参数,把已确认的入队写并进客户端打开的事务、被其 ROLLBACK 静默丢弃;bootstrap 任务由**看门狗**守护:任务死亡(panic)或退出后门未关 → 限时(60s)强排空、仍拿不到写路径则硬关门并响亮告警(门开着=无限确认永不落地的写,比报错危害大);drain 重放失败的条数进 REQ_STATUS 的 `replay_failures`(确认过的写丢失必须可观察)。**Applied 后做一次摘要复验**(`verify_join_convergence`:快照+队列重放只覆盖到达过的扇出,丢失的扇出曾静默分叉到下次重启才被发现;无任何可达 peer 同意就地转入 repair)。发起 holds 前先对各 peer 发 **id=0+自身身份的 REQ_RELEASE 清扫上一次尝试的残留 hold**(REQ_HOLD 响应丢失会让对端冻结到 60s 看门狗、期间重试全 busy);成功路径的 REQ_RELEASE 并行发送(串行把冻结期放大成 peers×超时)。加入前若本地已有客户端写入则放弃 bootstrap 保留本地数据(注册仍生效,后续写靠扇出收敛)。空集群(所有 peer 都 0 表)不互相 sync,静态 peers 配置即覆盖
- **重启反熵修复(rejoin repair,两阶段)**:持有数据的节点配置了 `DOCSQL_PEERS` 时,启动即并发探测各 peer 的表摘要(REQ_DIGEST,载荷为 `Database::digests()` 的 JSON:每表 行数 + 行哈希(逐行编码字节的序无关求和,堆布局无关)+ schema 哈希;系统表全跳过,schema 哈希刻意排除 pages/index_roots 等物理字段)与期刊窗口(REQ_STATUS 的 cluster_id/journal_head/journal_oldest)。与任一可达 peer 摘要一致 → 不修,直接 drain 关 sync 门。**阶段一:增量补齐(缺多少补多少)**——每个节点把本地提交的写按序记入 `_cluster_log`(节点内单调 seq;**记账与数据同提交**:`execute_sql` 把用户写与 `journal_append` 包进引擎「写单元」(`Database::write_unit`,RAII 守卫)——共享一次 WAL fsync,未融合时是每写两次引擎提交;写单元的单一 fsync 失败则整条语句报错不确认;**async_commit(DOCSQL_ASYNC_COMMIT=1)模式下单元照常分组记账,但 `end` 不强制 fsync**(引擎侧感知 async,留给后台 flusher 批量——否则每单元一次 fsync 会把组提交打回原形);**无扇出目标的节点——单节点、无 peers 无 replicate_to——跳过记期刊**,期刊永远无人拉取,加 peers 需重启生效,收敛走摘要→快照兜底),扇出用 REQ_SQL_SEQ 携带 (seq, node_id),接收方**应用与位点记账同样融合**(`execute_sql` 的 `seq_pos` 参数:应用写 + `_cluster_pos` 位点推进在同一写单元、一次 fsync);显式事务 COMMIT 后的 drain 把整批 journal 追加放进**一个**写单元(N 条一次 fsync;单元 end 失败整批退回无序号扇出,防 peer 采纳指向可能未落盘条目的位点)再逐条扇出;重入节点对每个原点按位点拉缺口(REQ_CATCHUP,只读不冻结),应用后重排空 sync 门再复验摘要,一致即收敛、**全程无快照无冻结**。边界:位点缺失(新加入原点/从未采纳过)、位点在窗口外(`DOCSQL_CATCHUP_WINDOW` 裁剪,默认 10 万条,每 512 条运行时裁剪)→ 增量不可行。**阶段二:快照兜底**——增量不可行或复验仍分歧时,按选举出的参考方快照整体重建(规则见下):复用 join 的 hold 冻结 → dump → 释放 → 流式回传,`apply_repair_sync` 在单事务内 `wipe_user_tables()`(绕过 DROP 的 FK 引用检查——快照里没有的本地表也要清掉)+ 回放,失败回滚保持原状;采纳成功后位点重建分两步:先清旧位点(`positions_clear`,**与快照同一事务**——曾是 COMMIT 后的独立提交,崩溃窗口留下的陈旧高位点会被重启时的 Converged 出口永久保留,而该出口既不清也不重播种),以探测头播种**下限**(drain 据此跳过快照已覆盖的带序号入队写),drain 后再**新探各原点当前 head 抬到位**(探测头早于快照传输,直接留用会让下次重入重放快照已覆盖的条目而必退化快照;join 的 `apply_sync` 同此序;新探仅在失去 write_order 后做,网络往返不挡本地写),之后的重入走增量;**采纳事务内同时 `journal_void_all()` 作废本机期刊全部条目的文本**(seq 原位保留、重放为 PRAGMA no-op)——被裁决丢弃的独有写若原文留在期刊,日后任何节点的 repair 都会按位点把它们拉回来「复活」成活数据;第三方拉到作废段会因摘要复验失败退到快照,与复活引发的多余快照轮次同价但无幽灵读。参考方选择(全网格确定性,各节点对同一摘要集合算出同一结论):peer 按相同摘要分组,最大组胜出——组内成员互见一致走「不修」,其余节点拉取(从组内任一成员拉结果相同);无多数方(各方可达状态互不相同,如两节点各持独有写)时按「行数多者优先,并列按摘要序列化序」选出唯一参考方,参考方照常服务、其余拉取,**与启动顺序无关**;**空组永不采纳**(本节点是唯一有数据的副本时不得因 peer 全空而清库);所有 peer 不可达则重试数轮再按本地服务。**代价:少数方/数据较少一方的独有写被参考方快照覆盖**——没有向量时钟/行级合并,分区分歧中少数侧的写不保留(部署测试第 10.5 章断言该策略)。修复只由重启触发:仅分区未重启不收敛,任一 divergent 节点重启即全网格收敛。安全性要点:位点只能落后于数据不能超前(推进一律走单调 `advance_position`——catchup 批量记账与实时扇出交错时,迟到的低位 seq 不得把位点压回已应用值,否则下次拉取重放该条、重复键错误退化快照;实时收写与位点已同单元原子落盘,崩溃只会造成重放冲突→快照兜底,绝不漏);期刊条目与数据写同单元提交(融合前的「数据已落、条目丢失」崩溃窗口已闭;catchup 回放批与 run_catchup 的收尾位点仍是独立提交,那两条路径的窗口由摘要复验兜底);日志/位点/身份表全部系统表化(排除摘要/快照;SQL 写入/DDL 恒拒、只读 SELECT 放行,控制台对象树以只读「系统表」分支展示)。sync 门对一切配置了 peers 的节点在启动期开放(join 与 repair 共用「快照 < 队列 < 直用」全序);修复轮数(20)刻意多于 fresh bootstrap(10),因为对端自身启动同步期间会拒绝服务快照,拉取方必须熬过对方的开门窗口。
...
- 消息存于引擎系统表 `_pubsub_messages`(常量 `core::engine::PUBSUB_TABLE`;server 启动时 `CREATE TABLE IF NOT EXISTS`),白得 WAL 持久化;对 SQL 客户端只读开放、写入/DDL 拒绝(`execute_sql` 的系统表门禁在写语句上按 **AST 写目标表名**分类(`stmt_write_targets`,来自 `parse_classified` 的单语句 AST):写系统表恒拒、字面量/注释里提到系统表名的用户写照常放行(旧子串扫描会误拒)、SELECT 放行——控制台「系统表」分支靠它查看;只读视图 `docsql_pubsub` 仍可用,重写标识符后走正常 SQL 路径);status/web 的表计数与对象树用户区均过滤系统表(对象树单列只读分支)
...
- `DOCSQL_WEB_AUTH_FILE`:web 控制台账号凭据文件路径。已设(非空)= 账号门激活:首次打开控制台强制设置用户名/密码(盐化 PBKDF2-HMAC-SHA256 存该文件,凭据文件创建即 0600;数据仍在节点;会话 HttpOnly Cookie 12h 滑动;登录失败锁定同 server 策略,默认按直连 IP 键控,反代部署设 `DOCSQL_WEB_TRUST_PROXY=1` 改按 X-Forwarded-For 首段;`DOCSQL_TOKEN` 仍可作 API 旁路),之后进入需登录。**setup 完成前数据端点一律 401**(旧「Setup 态匿名可达」窗口已收紧——整库替换级操作不得匿名触达;`/api/auth/*` 本身无需凭据)。未设或空 = 关闭账号门(legacy 仅 token 头;**run-tests.sh 显式置空依赖此语义**)。compose 两个文件均为 web 服务挂命名卷(`web-auth`/`web-auth-single`,项目前缀区分)并默认指向 `/auth/console-auth.json`;reset-data.sh 一并清除(恢复首次设置态)。
- `DOCSQL_WEB_COOKIE_SECURE=1`:会话 Cookie 附加 Secure 属性(TLS 反代终止后回源 http 的部署建议开启,防止会话经明文入口外泄);默认关闭。
- `DOCSQL_WEB_TRUST_PROXY=1`:登录失败锁定改按 X-Forwarded-For 首段键控(反代部署防「一人输错全员锁死」);仅在可信代理(会剥离/覆写该头)后面开启,默认关闭(按直连 IP)。
...
- `DOCSQL_BACKUP_INTERVAL_SECS`:自动备份间隔秒数(默认 86400 每日一次;0=关闭;**首拍仅在无备份或最新备份已超一个间隔时立即触发**——重启风暴不再每个进程生命期写一份近同快照挤掉 keep-N 里的历史时点)。备份 = `dump_script()` 逻辑快照(整库一致点、不含系统表),落在 `<db 目录>/backups/`(容器内 `/data/backups`,随数据卷持久);集群为每节点独立备份。恢复=重放备份文件(整库替换,重放期间持写路径,完成后做收敛自愈+验证),见 README。
- `DOCSQL_BACKUP_KEEP`:每节点保留的备份份数(默认 7,超出删最旧,只删 `backup-*.sql` 命名模式的文件)。
- `DOCSQL_BACKUP_DIR`:备份目录覆盖(默认 `<db 目录>/backups`)。
