...
| 文件 | 职责 |
|---|---|
| `lib.rs` / `main.rs` | tokio TCP 服务器:SQL 会话、REQ_AUTH 鉴权(客户端 token / 集群 token 双身份 `ConnRole`,复制帧仅 peer)、REQ_PROMOTE 故障转移提升、REQ_STATUS 节点状态报告(JSON)、REQ_LOGS 日志报告、REQ_DIGEST/REQ_SQL_SEQ/REQ_CATCHUP(catch-up 复制:摘要探测、带序号复制写、增量拉取)、REQ_BACKUP(备份管理:list/手动触发/restore 整库恢复,需认证;trigger 与 restore 拒只读连接,异步执行)、pub/sub 帧路由、副本协议 |
...
- 消息存于引擎系统表 `_pubsub_messages`(常量 `core::engine::PUBSUB_TABLE`;server 启动时 `CREATE TABLE IF NOT EXISTS`),白得 WAL 持久化;对 SQL 客户端只读开放、写入/DDL 拒绝(`execute_sql` 的系统表门禁按 AST 分类:写拦、SELECT 放行——控制台「系统表」分支靠它查看;只读视图 `docsql_pubsub` 仍可用,重写标识符后走正常 SQL 路径);status/web 的表计数与对象树用户区均过滤它(对象树单列只读分支)
...
- `DOCSQL_BACKUP_INTERVAL_SECS`:自动备份间隔秒数(默认 86400 每日一次;0=关闭;首拍即触发,重启即得新备份)。备份 = `dump_script()` 逻辑快照(整库一致点、不含系统表),落在 `<db 目录>/backups/`(容器内 `/data/backups`,随数据卷持久);集群为每节点独立备份。恢复=重放备份文件(整库替换),见 README。
