...
| 文件 | 职责 |
|---|---|
| `lib.rs` / `main.rs` | tokio TCP 服务器:SQL 会话、REQ_AUTH 鉴权(客户端 token / 集群 token 双身份 `ConnRole`,复制帧仅 peer)、REQ_PROMOTE 故障转移提升、REQ_STATUS 节点状态报告(JSON)、REQ_LOGS 日志报告、pub/sub 帧路由、副本协议 |
