...
1. **WAL/pager 顺序** — 一切落盘经 pager;先写 WAL 再应用页面;事务内的页面回写延迟到 WAL fsync 之后(防数据页领先日志)。动 pager 提交路径保持该顺序。checkpoint 已后台化(pager.rs:软阈值 8MB 请求后台线程 fsync 数据文件,硬阈值 64MB 写路径停等一次覆盖全日志的 fsync 后内联截断;ckpt 线程只 fsync 自有 dup 句柄,不碰 WAL/池/pending_writes;`covered_len` 的"低于它的每个 WAL 字节已落数据文件"成立依据 = 请求只出现在 `flush_pending` 排空之后——新增 `maybe_checkpoint`/`try_free_truncate` 调用点必须保持该顺序),**WAL 截断仍必须发生在覆盖全日志的数据文件 fsync 之后**;页数增长经 `persisted_pages` 记账即时持久化进页头,否则 WAL 截断后重启丢页数(2026-09-13 修复的既有缺陷)。**WAL 延迟提交围栏**:显式事务内每条语句以 `KIND_COMMIT_DEFERRED` 记账、SQL COMMIT 时 `sync_wal` 追加 `KIND_FENCE` 后再 fsync——恢复/快照物化只把 fence 之前的 deferred 提交视为已提交,进程被杀也不会重放「半个事务」(2026-09 修复);普通同步提交(`Wal::commit`)先补 fence 再提交,保证被泄漏写单元的已回写页面拥有可重放的 redo。`covered_len` 仍只在 `pending_writes` 排空后记账(`maybe_checkpoint`/`try_free_truncate` 已内置该守卫)。**页回收与事务回滚**:pager 维护内存可复用页池(`Pager::free_page` 随事务提交生效、abort 归还,页表/索引树重写与 TRUNCATE/DROP 前必须先 free 旧存储,新分配自动复用)——TRUNCATE/ROLLBACK/重写不再无界增长文件;显式事务的 `BEGIN`/`SAVEPOINT` 只做 catalog Arc 快照 + 页 undo 日志(`Pager::begin_undo`,`Set/Alloc/Realloc/Free` 按被改页记账,SAVEPOINT 落基线镜像,ROLLBACK 逆序回放),**别再退回 `snapshot_all` 全库深拷**;恢复路径禁止对 staled meta 走 `free_table_storage`/`collect_pages`(页可能已被本事务复用)。**pager 全方法 `&self`**(WAL/pending/页数/页 LSN 均内部锁;写串行仍由引擎写锁保证),供 MVCC 阶段 B 快照读共享:`begin_snapshot`/`read_page_as_of`/`end_snapshot` + WAL epoch(checkpoint 归零 LSN,快照按 epoch+LSN 定位;**截断在 WAL 锁内同时清 `page_lsn` 并发布 epoch 镜像**——LSN 空间归零后跨 epoch 比较无意义,残留旧值会把快路径读赶进无历史的慢路径伪报 `SnapshotTooOld`);**软截断在有活跃快照时让位**(快照的 as-of 页历史只在 WAL),硬阈值流控优先、照常截断,被截断的快照在其后响亮报 `SnapshotTooOld`,绝不静默回退新版本页;提交路径在 WAL 锁内完成「append+commit+读面应用」(池/pending/页 LSN,池锁最后释放),快照 begin 读 commit head 与页面可见性由此原子,**`read_page_as_of` 快路径的 seqlock 双检依赖该释放顺序**;慢路径物化持锁为零——独立只读句柄**流式**扫描(`Wal::frames`,两遍:committed 集合 → 按 LSN 直写缓存),扫后 WAL 锁内复验 epoch,别退回持 snaps/WAL 锁做全量 IO。**WAL 打开/崩溃恢复/快照物化一律流式,绝不 `read_to_end`/物化整条日志**:被杀死的大事务(如快照采纳)能把日志堆到数 GB,物化会在恢复/截断之前就 OOM,把节点卡进重启崩溃循环(2026-09-14 node-d 9.5GB WAL 实测;恢复两遍流式 = O(1) 内存 + committed txid 集,按 LSN 顺序直写页与 last-image 聚合等价);**ROLLBACK/abort 后也走 `maybe_checkpoint`**(死帧没有下一次 commit 来触发上界,曾让 WAL 无界增长)。**阶段 B 读级 = 无锁视图**:server 读级(`execute_read_sql`)只在微秒级读锁内 `Database::read_view()`(克隆 catalog Arc + 取快照),SELECT 全程锁外跑,读不阻塞写;SELECT 执行链整体在 `ReadCx`(pager 引用 + catalog 快照 + deadline + `Option<Snapshot>`)上,页读取统一走 `PageReader`(Current/as-of 双模,heap/btree 读函数收 `&PageReader`,写函数收 `&Pager`);catalog = `BTreeMap<String, Arc<TableMeta>>`,写路径改目录一律经 `Database::catalog_mut`(内部 `Arc::make_mut`,仅在 detached 读者持有旧版本时深拷贝)或整条 `Arc::new` 替换——**别退回对 Arc 的直接可变借用,也别对全库逐表 make_mut**(先定位目标表再定点);只读消费表元数据用 Arc 克隆借引用(`exec_insert`/`exec_merge` 形态),别整条深拷;写路径访问共享读助手走 `*_cx` shim(`table_docs_cx`/`index_probe_cx`/`matches_cx`/`load_from_cx`/`exec_query_cx`/`subst_expr_cx`);**嵌进 SQL 文本的字符串/标识符转义一律走 `stmt::sql_string_literal`/`sql_quote_ident`**,别再手抄 `replace` 副本;扫描「跳过字面量」一律走 `stmt::sql_literal_end`(绑定/视图重写/日志脱敏/词法四处同源),**别用 `to_lowercase()` 副本的字节下标去索引原文**——Unicode 大小写映射会改变字节长度(KELVIN SIGN 三字节→一字节),曾使 `redact_sql` 在任意语句上 panic(2026-09-13 修复)。
2. **编码 ≠ 排序** — `core/encode.rs` 只保证往返一致;排序统一走 `Value::cmp_values`(B+树/ORDER BY/DISTINCT)。新增值类型必须同时扩展编码与 `cmp_values`,否则索引序被破坏。DISTINCT/UNION 按编码字节判重:Int(3) 与 Float(3.0) 比较相等但不会互相去重。**DECIMAL(`Value::Decimal`,encode tag 8)与 BLOB(`Value::Bytes`)是精确标量**:算术/聚合/比较走十进制语义(混合运算 Decimal 优先于 Float,Float 按精确二进制值转换),复制/回写照必须经 `value_literal`(`CAST('…' AS DECIMAL)` / `x'…'`),线协议 JSON 用 `$dec`/`$bytes`/`$float`(非有限浮点)标记且 `json.rs` 双向对称解析——别退回裸数字字面量(重放会降级 Float)或把标记当普通字符串。Rust 侧小数用 `rust_decimal`(28~29 位有效数字),别引第二套十进制定点。**复合索引的键 = `Value::Array` 按列序**(cmp_values 逐元素字典序,设计见 `docs/design/001-composite-indexes.md`):index_roots 键已泛化为 root_key(单列=列名,复合=索引名,旧卷零迁移),一切索引键构造/唯一判定必须走 `TableMeta::index_columns_of` + `index_key_of` + `root_key_unique`,勿回退按列名直取;复合 UNIQUE 的树级判重不进 meta.unique;OR REPLACE/IGNORE 的位移机制只认约束列,复合唯一冲突直接报错(明示语义)。**`ORDER BY <索引键> + 常量 LIMIT/OFFSET` 走索引序窗口**(`ReadCx::ordered_index_window`):按树序只装载窗口内文档,不再全表扫描+全量排序(50 万行实测:首屏/浅页 ~30x,深 OFFSET ~40x,keyset 双向 ~50x);门禁=单表无 JOIN、ORDER BY 与某索引列序全等且方向单一、键列全在 `meta.not_null`(索引树不存 NULL 键,`index_key_of` 丢弃——nullable 键列必须回退,否则行会静默消失)、LIMIT 为常量(负 LIMIT 视为无上限,`LIMIT -1 OFFSET n` 的纯 Skip 也走窗口);WHERE 仅当能探测同一棵树时启用,**探针吞下整个 WHERE 才算 exact**(`probe_plan` 返回 exact 标记,必须与逐行 `matches` 语义一致:exact 时按 `skip+take` 截断树遍历、OFFSET 行只计数不解码、跳过逐行过滤;残余条件回退全候选边走边过滤);DESC 走 `scan_limited_rev`/`range_bounded_rev_limited` 反向遍历,别退回「全量物化 + reverse」;严格下界用 `range_bounded_excl_limited` 在遍历内丢弃等值 run,别在 capped 扫描后再 `retain`(等值 run 会吃光 cap)。**无索引 `ORDER BY`+有界 LIMIT 走键提取 top-K 窗口**(`ReadCx::unindexed_order_window`):只从编码字节提取排序字段(`encode::extract_field`,其余值按编码长度跳过、不物化——新增值类型必须同步扩展 `skip_value`/`extract_field`),堆内保留 `skip+take` 候选,`select_nth_unstable` 切掉跳过段后只排序窗口(50 万行实测:ORDER BY name 320→41ms,深 OFFSET 366→98ms);门禁=单表无 JOIN 无 WHERE、ORDER BY 全部解析为顶层裸字段(输出列名优先,`t.f` 回退 `f`,别名/序号/表达式回退)、LIMIT 有界;比较器与并列序必须与通用路径一致(`cmp_maybe_null` + 扫描序 `seq`,有差分测试守着)。**`SELECT COUNT(*) FROM t`(无 WHERE/GROUP BY/ORDER/LIMIT)走 `Heap::live_count` 免解码活槽计数**(50 万行 387→1.5ms):活槽 = 非空槽(文档恒为 object 根,`insert` 只收 `&Object`),虚拟兼容视图名不得命中快路径(`is_compat_view`)。其余回退通用路径。EF provider 的生成主键因此声明 NOT NULL(`SchemaSync.cs`)。**Timestamp 值域 0001..=9999 在全部构造点收口**(CAST 整型、`$ts` wire 解码、±毫秒算术):`TIMESTAMP_MIN/MAX_MS`;超域值没有规范文本形,`value_literal` 无法重放(dump 会响亮报错),别放开。谓词级 Str↔Timestamp 提升统一走 `cmp_coerced`(binop/BETWEEN/IN/CASE/IS DISTINCT FROM 四处同源),索引探针边界提升在 `promote_plan_bounds`(复合键逐元素,采样 min/max 决定带)。`CREATE INDEX … DESC` 显式报错(引擎只建 ASC 树;EF 对 IsDescending 模型抛 NotSupportedException)。**视图展开有 16 层预算 + 建视图传递闭包环检测**(`MAX_VIEW_EXPANSION_DEPTH`/`enter_view_expansion`):CREATE OR REPLACE 曾能造环打崩进程;预算与授权层展开上限协同——别只在一处放宽。
...
8. **非确定性生成值由写入节点定值下发** — auto-GUID 主键(PK 列 GUID/UUID/UNIQUEIDENTIFIER/UUIDV7 + AUTOINCREMENT)INSERT 省略/NULL 时引擎填 UUIDv7,并把语句回写成显式值(`Database::take_resolved_sql`,用户管理语句的密码哈希回写同槽),扇出与 `tx_pending` 一律用回写文本;auto-GUID 表 `INSERT ... SELECT` 显式报错。今后任何随机/时间类默认值同理(定值回写或确定性可重算),否则对端各自生成、静默分叉。`DEFAULT NOW()` 的定值点在**插入时**(exec_insert 应用 default 后回写整句 INSERT 到 resolved_sql,与 auto-GUID 同机制),DDL 文本不折叠(折叠会冻结成建表时刻);CHECK 含 wall-clock 一律拒建,MERGE 应用 wall-clock DEFAULT 显式报错(无回写机制就不放行);折叠器 `fold_wall_clocks` 跳注释/容忍 `NOW ()` 空格——改扫描逻辑必须同步 `mentions_wall_clock` 预筛。UUIDv7 节点内严格单调、时钟回退不回退。
9. **pub/sub 顺序契约** — PUBLISH 必须先经引擎写路径提交(WAL)再 notify 本地订阅者、再扇出 peers(对端只落盘+推本地,不再转发);SUBSCRIBE 注册后**锁外**回放(大 backlog 不再卡住发布与写路径),每轮在注册表锁内原子完成 `arm_filter`+水位采样(≤采样值的已从实时流丢弃、由本轮增量回放补齐,更新的走实时),`skip_through` 去重保证回放/实时不丢不重——动锁顺序前先读 `pubsub.rs` 模块注释。id 节点内单调(TRIM 强制 keep≥1,否则重启续推失锚),at-least-once,慢连接丢实时帧可凭 id 重订阅补回。**订阅必须专用连接 + 专职读线程**(CLI/DocsqlSubscriber 已是);普通一问一答连接会把 RESP_PUSH 当命令响应错位。
...
12a. **扇出熔断与 pubsub 保留** — 对端连续传输失败进入 2s 起步的指数退避(`peer_backoff`),退避期仍发送但只给 500ms 试用预算(重启的对端立即恢复并清熔断),应用层拒绝不触发;`_pubsub_messages` 按 20 万行/512MB 自截断(按 id 区间删除最旧,`MAX_STORE_ROWS`/`MAX_STORE_BYTES`),`REQ_STATUS` 增 `user_state` 字段(仅用户无数据表的节点也算持有状态,join 探测不再判空)。
...
14. **备份/恢复** — 备份 = `dump_script()` 逻辑快照(整库一致点、不含系统表),落 `<db>/backups/backup-<UTCms>.sql`(.tmp+rename 原子;**同名 `.sha256` 校验和 sidecar 随写,restore 前强校验,缺失容忍旧备份**;保留 KEEP 份只删本命名模式并连带 sidecar;首拍仅在无备份或最新已超一个间隔时触发)。**锁纪律:backup 状态锁从不与 write_order/engine 嵌套**;dump 持 `lock_engine_for_write` 取,文件 IO 在全部锁释放后做。恢复 = 逐条走正常写路径重放(autocommit;整场持 write_order,客户端写不会插入恢复流),完成后 catchup 自愈 + 逐一比对可达 peer 摘要,状态暴露 `converged`/`note`;跨节点互斥(探测 peers 的 REQ_STATUS);启动 sync 门未关时 tick/trigger/restore 跳过/拒绝;trigger/restore 拒只读连接;仅接受 `backup-*.sql` 裸文件名;同名表覆盖,备份后新建的表保留。**PITR 增量链有连续性审计**:`collect_pitr_entries` 逐条过滤 `seq > base_seq` 并对整链断号/重叠报错(要求重拍全量),导出侧 `pitr_cursor` 取增量段/基准的 max、`from` 记真实首 seq、发现裁剪洞即拒——动这段前先想清楚三方的错位场景。期刊另有 512MB 字节上限(`journal_trim_bytes`,与条数窗口并行;`DOCSQL_CATCHUP_WINDOW=0` 也不至于无界)。
...
16. **Web 控制台** — 自身零存储:启动参数/`DOCSQL_UPSTREAM`/首个 peer = 默认管理节点;`node` 参数只在 `DOCSQL_PEERS` 白名单内切换(SSRF 防护);`/api/parse` 恒本地静态检查;数据端点 in-band error(HTTP 200 + `{"error":...}`,传输层失败才 5xx,`api._do` 归一化);`console.html` 经 `include_str!`,改完必须重新 `cargo build`(浏览器/Playwright 验证)。账号门 `web/auth.rs`:自带 SHA-256/HMAC/PBKDF2(有已知答案测试,**勿引加密 crate 除非删除本实现**);`DOCSQL_WEB_AUTH_FILE` 空/未设 = 关闭(= API 开放;浏览器不再有令牌输入,控制台连节点一律用进程环境 `DOCSQL_TOKEN`,与 compose 下发值同源),已设 = 首次强制 setup,**setup 前数据端点一律 401**,`DOCSQL_TOKEN` 可 API 旁路。`POST /api/backup` 触发必须带 JSON 体(空 `{}` 即可;无体/表单请求 415,CSRF 防护),`POST /api/auth/change`:会话(或 token 旁路)+ 当前密码双门(会话防路过的人乱试,当前密码防被劫持的页面静默换凭据),当前密码错误计入同一按 IP 登录锁定;成功后 `Sessions::keep_only` 只留发起会话(改密码必须踢掉其它在线者;token 旁路调用无会话 = 全踢),新密码留空 = 只改用户名;写文件成功后才动内存副本(写失败旧凭据仍权威)。
17. **EF Core** — 独立原生提供程序(基于 Docsql ADO.NET,不依赖 SQLite),不重写关系生成;EnsureCreated/惰性建表与索引同步默认开启(模型增删表、索引自动同步,无需 Migrations),同步走「先校验后同步」:新连接先用两条一次性查询(`information_schema.columns` + `sqlite_master` 索引)确认模型要求的表/列/索引齐备,齐备即跳过;缺表/缺列/缺索引/unique 漂移/模型已移除的 `IX_` 索引才全量 `SyncModel` —— **别退回按连接全量同步**(EF 每 DbContext 一条新连接,Web 每请求一次上下文,全量同步是数百次往返);`Database.Migrate()` 显式报错;EF 事务内 SaveChanges 依赖 SAVEPOINT 有限语义——相关改动必须跑两个 dotnet 套件。
18. **不支持的 SQL 显式报错** — 窗口函数(OVER)/`WINDOW`/`QUALIFY`/DISTINCT ON/`NATURAL JOIN`/`LATERAL`/表函数/`TABLESAMPLE`/`FOR UPDATE|SHARE`/`SELECT INTO|TOP`/`FOR XML|JSON`/`SETTINGS`/`FORMAT`/ON CONFLICT DO UPDATE/ON DUPLICATE KEY UPDATE/自定义 TRIM 字符集/FK 的 ON DELETE|UPDATE 动作/相关子查询/无 GROUP BY 的 HAVING;`PRAGMA` 是有意接受并忽略的兼容垫片。已支持:WITH(非递归)/CTAS/ON CONFLICT DO NOTHING|REPLACE/`SELECT *, expr`/GROUPING SETS·ROLLUP·CUBE+`GROUPING()`/聚合 `FILTER (WHERE …)`/`FETCH … WITH TIES`/`FROM (VALUES …) AS t(cols)`/`IS [NOT] DISTINCT FROM`/量化比较 `> ANY|<> ALL`(非相关子查询,subst 阶段改写为比较链)。**子句级静默忽略是红线**:引擎不实现的 Select/Query/TableFactor 子句字段必须在 `reject_unsupported_select_clauses`/`reject_unsupported_query_clauses` 及 `load_table_factor` 门禁里拒绝(NATURAL 曾是 CROSS JOIN、LATERAL/FOR UPDATE/SELECT INTO/TABLESAMPLE 曾静默丢弃),新增语法先在解析/执行层报错,不要静默吞掉。**表级复合 PK/UNIQUE(曾按单列声明)、`GROUP BY … WITH ROLLUP` 修饰符(曾被丢弃)、T-SQL 变量 `@p`/`@@VAR`(曾静默 NULL)、`OUTPUT`/表提示 `WITH(...)`/`UPDATE·DELETE ORDER BY|LIMIT`(曾被丢弃)、`#` 临时表(曾建持久表)、索引 `INCLUDE`/`WHERE`/`USING`/存储选项(曾丢子句)** 均须显式报错,别退回静默降级;**`ON CONFLICT (cols)`/`ON CONSTRAINT` 为定向冲突**(只跳过目标唯一约束、其余冲突照常报错),别退回全局跳过。
19. **PK ≠ NOT NULL** — 主键当前不隐含 NOT NULL(与主流不同);动约束逻辑需全量回归约束测试。
20. **deploy 钉子与本机环境** — `deploy/multinode-test.sh` 断言固定 REST/协议字段,改协议/REST 先同步该脚本与 dotnet 客户端;分区重连必须 `docker network connect --alias node-c <net> docsql-c`(裸 connect 丢别名,DNS/healthcheck/扇出静默失效,restart 不恢复),节点侧查询走 127.0.0.1;`DOCSQL_DEV_IMAGE_TAG`(dev)与 `DOCSQL_IMAGE_TAG`(prod)刻意分开,`.env` 的 prod tag 不会泄漏到 dev;本机到 github.com:443 间歇阻断,推送失败用 `git -c http.version=HTTP/1.1 push` 重试;Docker 构建基于 mcr.microsoft.com/azurelinux(docker.io 不可达);dotnet 的 bin/obj 不入库,新项目沿用;镜像运行层必须预建全部 compose 挂载点并 `chown 1000:1000`(/data、/auth——全新命名卷的属主继承自镜像内同名目录,镜像缺该目录则挂载点归 root,uid 1000 进程写入即 os error 13,console 凭据写入曾栽在此)。**禁止在 Docker 构建容器里跑 cargo/建持久 target 命名卷**(曾以 `docsql-builder` 常驻容器 + `docsql-*-target` 卷累积 ~30GB debug 产物):本机开发直接本地 cargo;确需 Linux 环境验证用一次性 `docker run --rm` 且不挂持久 target 卷,或依靠 CI。
...
基准脚本在 `crates/docsql-core/examples/bench*.rs`:`bench`(综合:插入/点查/索引查/更新/删除/扫描)、`bench6`(逐条 fsync vs 异步组提交)、`bench_micro`(点查成本拆解)、`bench_write`(INSERT/UPDATE 成本拆解)、`bench_join`(hash join vs 嵌套循环对照)、`bench_page`(分页:首屏/深 OFFSET/keyset 双向/DESC/无索引对照,默认 50 万行,可传行数)。统一 `cargo run -p docsql-core --example <name> --release` 采数,debug 只做冒烟不下结论;新增/修改 bench 脚本同样过提交门禁(`--all-targets` 含 examples)。动 pager/WAL/执行器/锁/分配路径后评估性能,按以下标准:
