...
| 用户视图(CREATE VIEW)为只读命名查询 | 视图存于目录命名空间,SELECT 穿透(支持视图套视图、外层过滤组合);授权按视图名发放,读视图不需要基表授权(权限收口) | 无物化视图/写穿透(INSERT/UPDATE/DELETE/TRUNCATE/ALTER/INDEX 对视图显式报错);写语句经视图读源需要基表授权(fail-closed 展开);视图无独立统计,外层查询不走基表索引探测 |
...
- 数据完整性:WAL 先日志后数据、备份 sha256 校验和、**PITR 恢复到时间点**(基准备份 + 增量段)、
  恢复后跨节点摘要收敛验证、集群反熵自愈;
