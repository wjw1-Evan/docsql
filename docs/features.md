...
纯管理工具、**自身零存储**,所有数据操作以客户端身份转发到管理节点:

- 对象资源管理器(表/列/索引/键/系统视图/系统表)、多标签查询编辑器(SQL 高亮、
  F5 执行、Ctrl+F5 分析、批量多结果集)、数据网格(排序/分页/**行内编辑与删除**);
- 新建表 / 编辑表 / 插入文档(列编辑网格、JSON 批量插入、允许表外字段)、
  编写 CREATE/DROP 脚本、删除表;
- 仪表盘、集群状态页、日志页(数据/同步/错误 + 来源过滤 + 5s 刷新)、备份管理页
  (状态/列表/立即备份/一键恢复)、用户与角色页(建号授权、表级权限编辑);
- 节点切换(默认管理节点 ↔ `DOCSQL_PEERS` 白名单内任意节点);
