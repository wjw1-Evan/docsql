...
- **T-SQL 表达式层已支持**(见 SQL 参考 · T-SQL 兼容面):`SELECT TOP [WITH TIES]`、
  `[方括号]` 标识符、`N'…'` 字面量、字符串 `+` 拼接、位运算 `& | ^ ~`、
  `LIKE '[a-z]'/[^…]` 字符类、日期/字符串/数学/转换(CONVERT 带 style、TRY_CAST/TRY_CONVERT/PARSE)/
  逻辑(IIF/CHOOSE)/哈希(HASHBYTES MD5/SHA1/SHA2_256)函数族、`NEWID()`(SELECT/INSERT,写路径定值回写)、
...
| 类别 | 函数 |
|---|---|
| 聚合 | `COUNT` `COUNT_BIG` `SUM` `AVG` `MIN` `MAX` `GROUP_CONCAT` `STRING_AGG`,均支持 `DISTINCT` |
| 字符串 | `UPPER/UCASE` `LOWER/LCASE` `LENGTH/LEN`(LEN 不计尾随空格) `SUBSTR/SUBSTRING` `TRIM/LTRIM/RTRIM`(支持 `TRIM('ab' FROM x)` 字符集形式) `CONCAT`(NULL 当空串,`\|\|` 仍传染) `LEFT/RIGHT` `CHARINDEX` `REPLACE` `REPLICATE` `REVERSE` `SPACE` `STR` `QUOTENAME` `ASCII/CHAR/NCHAR/UNICODE` `CONCAT_WS` `TRANSLATE` `STUFF` `STRING_ESCAPE` `FORMAT` |
| 数值 | `ABS` `ROUND`(DECIMAL 精确、半离零、支持负位数;FLOAT 保持浮点) `FLOOR/CEILING` `POWER` `SQRT` `SQUARE` `EXP` `LOG` `LOG10` `SIGN` `PI` `SIN/COS/TAN/COT/ASIN/ACOS/ATAN/ATN2` `DEGREES/RADIANS` |
| 空值/条件 | `COALESCE` `IFNULL` `NULLIF` `NVL` `NVL2` `DECODE` `IIF` `CHOOSE` `ISNULL` |
| 时间 | `NOW()` `CURRENT_TIMESTAMP`(TIMESTAMP 值);T-SQL 族 `GETDATE/GETUTCDATE/SYSDATETIME/SYSUTCDATETIME`(UTC) `DATEADD/DATEDIFF/DATEDIFF_BIG/DATEPART/DATENAME` `YEAR/MONTH/DAY/DAYOFYEAR` `EOMONTH` `DATEFROMPARTS` 族 `ISDATE` `ISNUMERIC`;`TIMESTAMP '…'` 字面量、`CAST(... AS TIMESTAMP)`、`±` 毫秒算术(见 SQL 参考 · 时间值) |
| 转换 | `CAST` `TRY_CAST` `CONVERT(类型, 值[, style])` `TRY_CONVERT` `PARSE` `TRY_PARSE`(en-US);CONVERT 常用日期 style(23/101/112/120/121/126 等)双向 |
...
| 标识/哈希 | `NEWID/NEWSEQUENTIALID`(UUIDv7;仅 SELECT/INSERT,写路径回写字面量) `DB_NAME` `DB_ID` `SERVERPROPERTY` `CHECKSUM/BINARY_CHECKSUM` `HASHBYTES(MD5/SHA1/SHA2_256)` |
| JSON 点读 | `JSON_EXTRACT(doc,'$.a.b[0]')` `JSON_TYPE` `JSON_VALID`(坏文本/缺路径返回 NULL) |
| JSON 数组 | `JSON_ARRAY_CONTAINS(json_text, value)` 数组成员判定(EF 实体集合 `Contains` 的翻译目标;非数组/坏 JSON → false,NULL 文本 → NULL);表值函数 `STRING_SPLIT`/`OPENJSON`/`GENERATE_SERIES`(FROM 中,需别名) |
